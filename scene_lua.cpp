...
int gr_nh_sphere_cmd(lua_State* L)
{
  ...
  glm::vec3 pos1;
  get_tuple(L, 2, &pos1[0], 3);
  glm::vec3 pos2;
  get_tuple(L, 3, &pos2[0], 3);
  double radius = luaL_checknumber(L, 4);
  data->node = new GeometryNode(name, new NonhierSphere(pos1, pos2, radius));
...
extern "C"
int gr_glass_material_cmd(lua_State* L)
{
    GRLUA_DEBUG_CALL;
    ...
    double refractive_idx = luaL_checknumber(L, 3);

    data->material = new glassMaterial(transmittance, reflectivity, refractive_idx);
...
static const luaL_Reg grlib_functions[] = {
  ...
  // New for final project
   {"glass_material", gr_glass_material_cmd},
  {"cone", gr_cone_cmd},
